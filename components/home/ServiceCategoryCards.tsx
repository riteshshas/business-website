...
import {
  serviceSelectorCategories,
  type ServiceSelectorCategory,
} from "@/data/services/service-selector-categories";
...
function CardContent({ category }: { category: Category }) {
  return <><span className="rounded-lg bg-slate-100 p-3 text-slate-700">{icons[category.title]}</span><span className="min-w-0 flex-1"><span className="block font-semibold text-slate-900">{category.title}</span><span className="mt-2 block text-sm leading-6 text-slate-600">{category.description}</span></span><svg aria-hidden="true" viewBox="0 0 24 24" className="mt-1 h-5 w-5 shrink-0 text-slate-400 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14" /><path d="m13 5 7 7-7 7" /></svg></>;
}

export default function ServiceCategoryCards({ categories }: { categories: Category[] }) {
  const [selectedCategory, setSelectedCategory] = useState<ServiceSelectorCategory | null>(null);
  const router = useRouter();

  const selectorCategoryByTitle = new Map(
    serviceSelectorCategories.map((category) => [category.title, category]),
  );

  return <><div className="mt-6 grid gap-4 sm:grid-cols-2">{categories.map((category) => {
    const selectorCategory = selectorCategoryByTitle.get(category.title);
    return selectorCategory ? <button key={category.title} type="button" onClick={() => setSelectedCategory(selectorCategory)} className={cardClassName}><CardContent category={category} /></button> : <Link key={category.title} href={category.href} className={cardClassName}><CardContent category={category} /></Link>;
  })}</div>{selectedCategory && <ServiceSelector title={`Choose a ${selectedCategory.title.toLowerCase()} service`} description={selectedCategory.description} services={selectedCategory.services} isOpen onClose={() => setSelectedCategory(null)} onSelect={(service) => router.push(`${selectedCategory.routeBase}/${service.slug}`)} />}</>;
}
